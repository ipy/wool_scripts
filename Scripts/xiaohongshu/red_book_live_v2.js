...
// 两条重写搭配使用才可以
// https://edith.xiaohongshu.com/api/sns/v2/note/feed url script-response-body https://github.com/ipy/wool_scripts/raw/main/Scripts/xiaohongshu/xiaohongshu.js
// https://edith.xiaohongshu.com/api/sns/v1/note/live_photo/save url script-response-body https://github.com/ipy/wool_scripts/raw/main/Scripts/xiaohongshu/red_book_live_v2.js
...
function replaceUrlContent(collectionA, collectionB) {
  ...
  collectionA.forEach((itemA) => {
    const matchingItemB = collectionB.find(
      (itemB) => itemB.file_id === itemA.file_id
    );
    if (matchingItemB) {
      itemA.url = itemA.url.replace(
        /(.*)\.mp4/,
        `${matchingItemB.url.match(/(.*)\.mp4/)[1]}.mp4`
      );
      itemA.author = "@fmz200";
    }
  });
