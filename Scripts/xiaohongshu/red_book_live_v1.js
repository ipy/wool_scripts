...
// https://edith.xiaohongshu.com/api/sns/v1/note/live_photo/save url script-response-body https://github.com/ipy/wool_scripts/raw/main/Scripts/xiaohongshu/red_book_live_v1.js
...
function process() {
  ...
  if (req_url.includes(url01)) {
    const myRequestCache = JSON.parse(
      $prefs.valueForKey("fmz200.xiaohongshu.feed")
    );
    console.log("读取缓存key：fmz200.xiaohongshu.feed.req");
    $task.fetch(myRequestCache).then(
      (response) => {
