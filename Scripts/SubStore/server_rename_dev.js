...
// 重要提示：该脚本是测试脚本，请使用 https://raw.githubusercontent.com/ipy/wool_scripts/main/Scripts/rename_simple.js
// 脚本地址：https://github.com/ipy/wool_scripts/raw/main/Scripts/SubStore/server_rename_dev.js
...
const DELIMITER = "|"; // 分隔符
const { isLoon, isSurge, isQX, isNode, isStash, isShadowRocket } =
  $substore.env;
// 节点转换的目标类型
...
async function operator(proxies) {
  ...
  const support =
    isLoon || (isSurge && parseInt($environment["surge-build"]) >= 2000);
  if (!support) {
  ...
  for (const groupKey in groups) {
    if (groups.hasOwnProperty(groupKey)) {
      ...
      group.forEach((item, index) => {
        item.name = `${item.name}${DELIMITER}${index < 10 ? "0" : ""}${
          index + 1
        }`;
      });
    }
